...
class ArvoreAVL:
    ...
    def _deletar_recursivo(self, no_atual, chave):
        ...
        if chave < no_atual.chave:
            ...
            if no_atual.esquerda is None or no_atual.direita is None:
                # Um filho ou nenhum
                temp = no_atual.esquerda if no_atual.esquerda is not None else no_atual.direita
                if temp is None:
                    # Sem filho
                    no_atual = None
                else:
                    # Um único filho: substitui pelo filho
                    no_atual = temp
            else:
                # Dois filhos: pega o sucessor (menor da subárvore direita)
        ...
        # Passo 4: Aplica rotações conforme necessário
    ...
    # --- Métodos utilitários opcionais (para depuração/demonstração) ---
    def em_ordem(self):
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Percurso em ordem (crescente):", arvore_avl.em_ordem())
